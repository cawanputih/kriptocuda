...
__global__ void kernelenk(big *m, big *k, big *g, big *p, big *y, big *res){
	...
	big* lm = (big*) malloc(sizeof(big));
	big* lk = (big*) malloc(sizeof(big));
	big* lg = (big*) malloc(sizeof(big));
	big* lp = (big*) malloc(sizeof(big));
	big* ly = (big*) malloc(sizeof(big));
	big* lres = (big*) malloc(sizeof(big) * 2);
	...
	lm->value = (uint*) malloc(sizeof(uint) * lm->size);
	lk->value = (uint*) malloc(sizeof(uint) * lk->size);
	lg->value = (uint*) malloc(sizeof(uint) * lg->size);
	lp->value = (uint*) malloc(sizeof(uint) * lp->size);
	ly->value = (uint*) malloc(sizeof(uint) * ly->size);
	lres[0].value = (uint*) malloc(sizeof(uint) * 8);
	lres[1].value = (uint*) malloc(sizeof(uint) * 8);

	for (int i = 0; i < 4; i++)
	{
		lm->value[i] = m[idx].value[i];
		lk->value[i] = k[idx].value[i];
		lg->value[i] = g[0].value[i];
		lp->value[i] = p[0].value[i];
		ly->value[i] = y[0].value[i];
	}
...
//	enkripsi(sm + jdx, sk + jdx, lg, lp, ly, sres + 2*jdx, minbuff, mulbuff);
	enkripsi(sm + jdx, sk + jdx, &sg, &sp, &sy, lres, minbuff, mulbuff);
//			enkripsi(lm, lk, lg, lp, ly, sres + 2*jdx, minbuff, mulbuff);
	// }
	if(idx == 0){
		printf("Semangkas\n");
		printf("xx[0].size = %u\n", sgval[0]);
	...
	res[2*idx].size = lres[0].size;
	res[2*idx+1].size = lres[1].size;

	for (int i = 0; i < lres[0].size; i++)
	{
		res[2*idx].value[i] = lres[0].value[i];
	}

	for (int i = 0; i < lres[1].size; i++)
	{
		res[2*idx+1].value[i] = lres[1].value[i];
	}
