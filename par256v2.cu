...
__global__ void kernelenk(big *m, big *k, big *g, big *p, big *y, big *res){
	...
	__shared__ big sg;
	__shared__ big sy;
	__shared__ uint s[8216];

	uint *sresval = s;
	uint *smulbuffval = (uint*)&sresval[16*128*2];
	uint *spval = (uint*)&smulbuffval[16*128];
	uint *sgval = (uint*)&spval[8];
	uint *syval = (uint*)&sgval[8];
	uint *smval = (uint*)&syval[8];
	uint *skval = (uint*)&smval[8*128];
	...
	for (int i = 0; i < 8; i++)
	{
		smval[jdx*8+i] = m[idx].value[i];
		skval[jdx*8+i] = k[idx].value[i];
		spval[i] = p[0].value[i];
		sgval[i] = g[0].value[i];
		syval[i] = y[0].value[i];
	}

	sm[jdx].value = (uint*)&smval[jdx*8];
	sk[jdx].value = (uint*)&skval[jdx*8];
	sres[2*jdx].value = (uint*)&sresval[jdx*16*2];
	sres[2*jdx+1].value = (uint*)&sresval[jdx*16*2+16];
	smulbuff[jdx].value = (uint*)&smulbuffval[jdx*16];
	sp.value = spval;
	sg.value = sgval;
	sy.value = syval;
	...
	// mulbuff->value = (uint*) malloc(sizeof(uint) * sp.size * 2);
			...
			enkripsi(sm + jdx, sk + jdx, &sg, &sp, &sy, sres + 2*jdx, minbuff, smulbuff+jdx);
	// }
	// if(idx == 0){
	// 	printf("Semangkas\n");
	// 	printf("xx[0].size = %u\n", sgval[0]);
...
__global__ void kerneldek(big *c, big *e, big *p, big *res){
	...
	uint *sresval = s;
	uint *spval = (uint*)&sresval[16*128];
	uint *saval = (uint*)&spval[8];
	uint *sbval = (uint*)&saval[8*128];
	uint *seval = (uint*)&sbval[8*128];
	...
	for (int i = 0; i < 8; i++)
	{
		saval[jdx*8+i] = c[2*idx].value[i];
		sbval[jdx*8+i] = c[2*idx+1].value[i];
		spval[i] = p[0].value[i];
		seval[i] = e[0].value[i];
	}

	sa[jdx].value = (uint*)&saval[jdx*8];
	sb[jdx].value = (uint*)&sbval[jdx*8];
	sres[jdx].value = (uint*)&sresval[jdx*16];
	sp.value = spval;
	se.value = seval;
