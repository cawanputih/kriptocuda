...
__global__ void kernelenk(big *m, big *k, big *g, big *p, big *y, big *res){
	...
	__shared__ big sg;
	__shared__ big sy;
	__shared__ uint s[9252];

	uint *sresval = s;
	uint *spval = (uint*)&sresval[24*128*2];
	uint *sgval = (uint*)&spval[12];
	uint *syval = (uint*)&sgval[12];
	uint *smval = (uint*)&syval[12];
	uint *skval = (uint*)&smval[12*128];
	...
	for (int i = 0; i < 12; i++)
	{
		smval[jdx*12+i] = m[idx].value[i];
		skval[jdx*12+i] = k[idx].value[i];
		spval[i] = p[0].value[i];
		sgval[i] = g[0].value[i];
		syval[i] = y[0].value[i];
	}

	sm[jdx].value = (uint*)&smval[jdx*12];
	sk[jdx].value = (uint*)&skval[jdx*12];
	sres[2*jdx].value = (uint*)&sresval[jdx*24*2];
	sres[2*jdx+1].value = (uint*)&sresval[jdx*24*2+24];
	sp.value = spval;
	sg.value = sgval;
	sy.value = syval;
...
__global__ void kerneldek(big *c, big *e, big *p, big *res){
	...
	__shared__ big sp;
	__shared__ big se;
	__shared__ uint s[6168];

	uint *sresval = s;
	uint *spval = (uint*)&sresval[24*128];
	uint *saval = (uint*)&spval[12];
	uint *sbval = (uint*)&saval[12*128];
	uint *seval = (uint*)&sbval[12*128];
	...
	for (int i = 0; i < 12; i++)
	{
		saval[jdx*12+i] = c[2*idx].value[i];
		sbval[jdx*12+i] = c[2*idx+1].value[i];
		spval[i] = p[0].value[i];
		seval[i] = e[0].value[i];
	}

	sa[jdx].value = (uint*)&saval[jdx*12];
	sb[jdx].value = (uint*)&sbval[jdx*12];
	sres[jdx].value = (uint*)&sresval[jdx*24];
	sp.value = spval;
	se.value = seval;
