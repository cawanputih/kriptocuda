...
void init(big *p, big *g, big *x, big*e, big *y, big *m, big *k, big *res){
	...
	p->size = 2;
	cudaMallocManaged(&(p->value),p->size * sizeof(uint));
	p->value[0] = UINT_MAX;
	...
	// Kunci publik g
	g->size = 2;
	cudaMallocManaged(&(g->value),g->size * sizeof(uint));
	...
	// Kunci privat x
	x->size = 2;
	cudaMallocManaged(&(x->value),x->size * sizeof(uint));
	...
	cudaMallocManaged(&(e->value),p->size * sizeof(uint));
	...
	cudaMallocManaged(&(y->value),p->size * 2 * sizeof(uint));
	...
	for(int i = 0 ; i < banyakdata ; i++){
		m[i].size = 2;
		cudaMallocManaged(&(m[i].value),m[i].size * sizeof(uint));
		...
		k[i].size = 2;
		cudaMallocManaged(&(k[i].value),k[i].size * sizeof(uint));
	...
	for (int i = 0; i < banyakdata*2; i++)
	{
		cudaMallocManaged(&(res[i].value),p->size * sizeof(uint));
	}
}
...
int main(){
	...
	cudaMallocManaged(&p,sizeof(big));
	cudaMallocManaged(&g,sizeof(big));
	cudaMallocManaged(&x,sizeof(big));
	cudaMallocManaged(&e,sizeof(big));
	cudaMallocManaged(&y,sizeof(big));
	cudaMallocManaged(&m,banyakdata * sizeof(big));
	cudaMallocManaged(&k,banyakdata * sizeof(big));
	cudaMallocManaged(&res,banyakdata * 2 *sizeof(big));
