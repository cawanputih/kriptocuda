...
__global__ void kernelenk(big *m, big *k, big *g, big *p, big *y, big *res){
	...
	__shared__ big sminbuff[128];
	__shared__ big sp;
	__shared__ big sg;
	__shared__ big sy;
	__shared__ uint s[9000];

	uint *sresval = s;
	uint *sminbuffval = (uint*)&sresval[20*128*2];
	uint *spval = (uint*)&sminbuffval[10*128];
	uint *sgval = (uint*)&spval[10];
	uint *syval = (uint*)&sgval[10];
	uint *smval = (uint*)&syval[10];
	uint *skval = (uint*)&smval[10*128];
	...
	for (int i = 0; i < 10; i++)
	{
		smval[jdx*10+i] = m[idx].value[i];
		skval[jdx*10+i] = k[idx].value[i];
		spval[i] = p[0].value[i];
		sgval[i] = g[0].value[i];
		syval[i] = y[0].value[i];
	}

	sm[jdx].value = (uint*)&smval[jdx*10];
	sk[jdx].value = (uint*)&skval[jdx*10];
	sres[2*jdx].value = (uint*)&sresval[jdx*20*2];
	sres[2*jdx+1].value = (uint*)&sresval[jdx*20*2+20];
	sminbuff[jdx].value = (uint*)&sminbuffval[jdx*10];
	sp.value = spval;
	sg.value = sgval;
	sy.value = syval;
	...
	mulbuff->value = (uint*) malloc(sizeof(uint) * sp.size * 2);
			...
			enkripsi(sm + jdx, sk + jdx, &sg, &sp, &sy, sres + 2*jdx, sminbuff + jdx, mulbuff);
	// }
	if(idx == 0){
		printf("Semangkas\n");
		printf("xx[0].size = %u\n", sgval[0]);
...
__global__ void kerneldek(big *c, big *e, big *p, big *res){
	...
	__shared__ big sp;
	__shared__ big se;
	__shared__ uint s[6500];

	uint *sresval = s;
	// uint *sminbuffval = (uint*)&sresval[20*128];
	uint *spval = (uint*)&sresval[20*128];
	uint *saval = (uint*)&spval[10];
	uint *sbval = (uint*)&saval[10*128];
	uint *seval = (uint*)&sbval[10*128];
	...
	for (int i = 0; i < 10; i++)
	{
		saval[jdx*10+i] = c[2*idx].value[i];
		sbval[jdx*10+i] = c[2*idx+1].value[i];
		spval[i] = p[0].value[i];
		seval[i] = e[0].value[i];
	}

	sa[jdx].value = (uint*)&saval[jdx*10];
	sb[jdx].value = (uint*)&sbval[jdx*10];
	sres[jdx].value = (uint*)&sresval[jdx*20];
	// sminbuff[jdx].value = (uint*)&sminbuffval[jdx*10];
	sp.value = spval;
	se.value = seval;
